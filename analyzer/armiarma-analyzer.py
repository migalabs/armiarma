...
# Base directory for Armiarma repo
ARMIARMA_DIR = Path(__file__).resolve().parent.parent
...
def main():
	...
	folder = 'examples/' + sys.argv[1]

	csvFile = os.path.join(ARMIARMA_DIR, (folder + '/metrics.csv'))
	outputFigsFolder = os.path.join(ARMIARMA_DIR, (folder + '/figs'))
	pdfFile = os.path.join(ARMIARMA_DIR, (folder + '/metrics-summary.pdf'))

	rumorMetricsPanda = pd.read_csv(csvFile)

	if path.exists(outputFigsFolder):
		pass
	else:
		os.mkdir(outputFigsFolder)
	...
	### The new metrics needs to be filled, the blank variables that
	### then new version exports can't be empty on plotFromPandas
	rumorMetricsPanda = rumorMetricsPanda.fillna('Unknown')
	### End of the filling
	...
	for index, row in rumorMetricsPanda.iterrows():
		if '/13000' in row["Address"]:
			cnt13000 = cnt13000 +1
		elif '/9000' in row["Address"]:
			cnt9000 = cnt9000 +1
		else:
