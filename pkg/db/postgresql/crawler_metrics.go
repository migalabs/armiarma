...
func (db *DBClient) GetClientDistribution() (map[string]interface{}, error) {
	...
	rows, err := db.psqlPool.Query(
		...
		FROM peer_info
		WHERE 
			deprecated = 'false' and attempted = 'true' and client_name IS NOT NULL and to_timestamp(last_activity) > CURRENT_TIMESTAMP - INTERVAL '$1 DAY';
...
func (db *DBClient) GetHostingDistribution() (map[string]interface{}, error) {
	...
	err := db.psqlPool.QueryRow(
		...
		FROM (
			...
			WHERE pi.deprecated='false' and attempted = 'true' and client_name IS NOT NULL and ips.mobile='true' and to_timestamp(last_activity) > CURRENT_TIMESTAMP - INTERVAL '$1 DAY'
		) as aux
		`,
		LastActivityValidRange,
	).Scan(&mobile)
	...
	err = db.psqlPool.QueryRow(
		...
		FROM (
			...
			FROM peer_info as pi
			INNER JOIN ips ON pi.ip=ips.ip
			WHERE pi.deprecated='false' and attempted = 'true' and client_name IS NOT NULL and ips.proxy='true' and to_timestamp(last_activity) > CURRENT_TIMESTAMP - INTERVAL '$1 DAY'
		) as aux
		`,
		LastActivityValidRange,
	).Scan(&proxy)
	...
	err = db.psqlPool.QueryRow(
		...
		FROM (
			...
			FROM peer_info as pi
			INNER JOIN ips ON pi.ip=ips.ip
			WHERE pi.deprecated='false' and attempted = 'true' and client_name IS NOT NULL and ips.hosting='true' and to_timestamp(last_activity) > CURRENT_TIMESTAMP - INTERVAL '$1 DAY'
		) as aux		
		`,
		LastActivityValidRange,
	).Scan(&hosted)
	...
	rows, err := db.psqlPool.Query(
		...
		FROM (
			SELECT
				...
				END as latency    
			FROM peer_info 
			WHERE deprecated=false and client_name IS NOT NULL and to_timestamp(last_activity) > CURRENT_TIMESTAMP - INTERVAL '$1 DAY'
		) as t 
		GROUP BY t.latency 
		ORDER BY nodes DESC;	
		`,
