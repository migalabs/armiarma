...
func (mdf MetricsDataFrame) ExportToCSV(filePath string) error {
	...
	_, err = csvFile.WriteString("Peer Id,Node Id,Client,Version,Pubkey,Address,Ip,Country,City,Latency,Connections,Disconnections,Connected Time,Beacon Blocks,Beacon Aggregations,Voluntary Exits,Proposer Slashings,Attester Slashings\n")
	if err != nil {
	...
	for idx := 0; idx < mdf.Len; idx++ {
		var csvRow string
		csvRow = mdf.PeerIds.GetByIndex(idx).String() + "," + mdf.NodeIds.GetByIndex(idx) + "," + mdf.ClientTypes.GetByIndex(idx) + "," +
			mdf.ClientVersions.GetByIndex(idx) + "," + mdf.PubKeys.GetByIndex(idx) + "," + mdf.Addresses.GetByIndex(idx) + "," + mdf.Ips.GetByIndex(idx) + "," +
			mdf.Countries.GetByIndex(idx) + "," + mdf.Cities.GetByIndex(idx) + "," + strconv.Itoa(int(mdf.Latencies.GetByIndex(idx))) + "," + strconv.Itoa(int(mdf.Connections.GetByIndex(idx))) + "," +
			strconv.Itoa(int(mdf.Disconnections.GetByIndex(idx))) + "," + strconv.Itoa(int(mdf.ConnectedTimes.GetByIndex(idx))) + "," + strconv.Itoa(int(mdf.RBeaconBlocks.GetByIndex(idx))) + "," + strconv.Itoa(int(mdf.RBeaconAggregations.GetByIndex(idx))) + "," +
